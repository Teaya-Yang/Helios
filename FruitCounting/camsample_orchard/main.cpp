...
using namespace std;
  ...
  int main( void ){
     ...
     for (int i_orch = 1; i_orch < 3; i_orch++){ //Here, we only use orchard 0 as example
          ...
          for (int i_xcam = 0; i_xcam < 1; i_xcam++){// x is side by side distance - center is at 3.8
               float cam_x = 3.8f;
               for (int i_ycam = 0; i_ycam < 19; i_ycam++){// y is the direction of flight
                    float cam_y = -7.0f + 1.0f*i_ycam;
                    for (int i_zcam = 0; i_zcam < 15; i_zcam++){// z is the height
                         float cam_z = 1.0f + 0.5f*i_zcam;
                         ...
                         // // Right Facing Camera:
                         // CamCorners[1] = vec3(10,-12.02,9.08) + camShift;
                         // CamCorners[2] = vec3(10,12.02,9.08) + camShift;
                         // CamCorners[3] = vec3(10,-12.02,-9.08) + camShift;
                         // CamCorners[4] = vec3(10,12.02,-9.08) + camShift;
                         // cout<<"RC\n";
                         ...
                         // CamCorners[1] = vec3(12.02,10,9.08)*0.75 + camShift;
                         // CamCorners[2] = vec3(-12.02,10,9.08)*0.75 + camShift;
                         // CamCorners[3] = vec3(12.02,10,-9.08)*0.75 + camShift;
                         // CamCorners[4] = vec3(-12.02,10,-9.08)*0.75 + camShift;
                         ...
                         for (int i_tree = 0; i_tree < 4; i_tree++){
                              ...
                              for (int i = 0; i<raydir.size(); i++){
                                   ...
                                   /*Check first two trees for occlusion only if: 
                                   1) the ray has to do with the other two trees
                                   2) the camera is placed before y=3 */ 
                                   /*
                                   if (i_tree > 1 && cam_y < 3.0){
                                        for (int i_occtree = 0; i_occtree < 2; i_occtree++){
                                             //Iterating through each triangle
                                             // Leaves
                                             for (int j = 0; j<n_leafTri_array.at(i_occtree); j++){
                                                  vec3 v0 = leaf_vertices_array.at(i_occtree).at(j*3);
                                                  vec3 v1 = leaf_vertices_array.at(i_occtree).at(j*3+1);
                                                  vec3 v2 = leaf_vertices_array.at(i_occtree).at(j*3+2);
                                                  float t = intersectTriangle(v0,v1,v2,raydir.at(i),CamCorners.at(0));
                                                  if (t<tmin){
                                                       tmin = t;
                                                  }
                                             ...
                                             // Branches
                                             for (int j = 0; j<n_branchTri_array.at(i_occtree); j++){
                                                  vec3 v0 = branch_vertices_array.at(i_occtree).at(j*3);
                                                  vec3 v1 = branch_vertices_array.at(i_occtree).at(j*3+1);
                                                  vec3 v2 = branch_vertices_array.at(i_occtree).at(j*3+2);
                                                  float t = intersectTriangle(v0,v1,v2,raydir.at(i),CamCorners.at(0));
                                                  if (t<tmin){
                                                       tmin = t;
                                                  }
                                             ...
                                             // Trunk
                                             for (int j = 0; j<n_trunkTri_array.at(i_occtree); j++){
                                                  vec3 v0 = trunk_vertices_array.at(i_occtree).at(j*3);
                                                  vec3 v1 = trunk_vertices_array.at(i_occtree).at(j*3+1);
                                                  vec3 v2 = trunk_vertices_array.at(i_occtree).at(j*3+2);
                                                  float t = intersectTriangle(v0,v1,v2,raydir.at(i),CamCorners.at(0));
                                                  if (t<tmin){
                                                       tmin = t;
                                                  }
